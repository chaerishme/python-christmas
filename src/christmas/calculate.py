class Calculate:
    ...
    def calc_day_dc(self, day, orders):
        if day:
            main_count = sum(count for menu, count in orders.items() if menu in self.main)
            return main_count * 2023
        
        dessert_count = sum(count for menu, count in orders.items() if menu in self.dessert)
